<?php

return [
    'add_node' => 'Lägg till utlämningsplats',
    'address' => 'Adress',
    'administrators' => 'Administratörer',
    'agree' => 'OK',
    'all_producers' => 'Alla producenter',
    'all_users' => 'Alla användare',
    'block_producer' => 'Blockera producent',
    'cancel' => 'Avbryt',
    'cancel_invite' => 'Ta bort inbjudan',
    'city' => 'Stad',
    'change_weekday_warning' => 'Om du ändrar veckodag, interval eller startdatum kommer alla kopplade produkter försvinna eftersom produktens valda leveransdagarna inte längre matchar.',
    'confirm_delete' => 'Bekräfta ta bort',
    'confirm_delete_info' => 'Du är på väg att ta bort en nod. Är du säker på att du vill fortsätta?',
    'create_event' => 'Skapa evenemang',
    'date' => 'Datum',
    'delete' => 'Ta bort',
    'delete_node' => 'Ta bort nod',
    'deliveries' => 'Utlämningar',
    'delivery_interval' => 'Utlämningsinterval',
    'delivery_nodes' => 'Utlämningsplatser',
    'delivery_startdate' => 'Startdatum',
    'edit_delivery_settings' => 'Redigera utlämningsinställingarna',
    'edit_node' => 'Redigera nod',
    'email' => 'Email',
    'events' => 'Evenemang',
    'facebook_page' => 'Facebook-sida',
    'facebook_page_producers' => 'Facebook-sida för producenter',
    'guests' => 'Gäster',
    'information' => 'Information',
    'information_placeholder' => 'Information om din nod',
    'invite_admin' => 'Bjud in admin',
    'leave_node' => 'Lämna nod',
    'name' => 'Namn',
    'name_node' => 'Namnge din utlämningsplats',
    'nearby_nodes' => 'Närliggande utlämningsplatser',
    'node_info' => 'Nod-information',
    'no_nodes_selected' => 'Du har inte valt några noder.',
    'node_terms' => 'Nod - Villkor',
    'node_added' => 'Utlämningsplats har lagts till.',
    'node_removed' => 'Utlämningsplats har tagits bort.',
    'no_events' => 'Inga evenemang.',
    'no_producers' => 'Inga producenter har anslutet till din nod.',
    'no_users' => 'Inga användare har anslutet till din nod.',
    'producers' => 'Producenter',
    'remove_node' => 'Ta bort utlämningsplats',
    'save_node' => 'Spara nod',
    'save_settings' => 'Spara inställningar',
    'select_interval' => 'Välj interval',
    'select_weekday' => 'Välj veckodag',
    'show_address_on_map' => 'Visa adress på karta',
    'time' => 'Tid',
    'time_placeholder' => 'Leveranstid',
    'unblock_producer' => 'Avblockera producent',
    'users' => 'Användare',
    'weekday' => 'Veckodag',
    'zip' => 'Postnummer',

    'monday' => 'Måndag',
    'tuesday' => 'Tisdag',
    'wednesday' => 'Onsdag',
    'thursday' => 'Torsdag',
    'friday' => 'Fredag',
    'saturday' => 'Lördag',
    'sunday' => 'Söndag',

    // How does it work
    'how_does_it_work' => 'Så här gör du',
    'hdiw_intro' => 'Så här skapar du en lokal utlämningsplats.',
    'hdiw_item_1' => 'Läs igenom, förstå och godkänn villkoren för att skapa och administrera en lokal utlämningsplats.',
    'hdiw_item_2' => 'Namnge och skapa din utlämningsplats i formuläret till vänster.',
    'hdiw_item_3' => 'Bjud in producenter till din utlämningsplats.',
    'hdiw_item_4' => 'Marknadsför utlämningsplatsen mot konsumenter genom sociala medier, ett pressmeddelande och kanske arrangera ett trevligt lokalt premiärevent.',
    'hdiw_outro' => 'Läs mer i FAQ för utlämningsplatser.',

    'week_1' => 'Varje vecka',
    'week_2' => 'Varannan vecka',
    'week_3' => 'Var tredje vecka',
    'week_4' => 'Var fjärde vecka',
    'month_1' => 'En gång i månaden',
];
